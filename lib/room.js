...
Room.last = function(limit, f){
    var out = [];
    for(var roomId in Room.all){
        if(out.length >= limit)
            break;

        out.push(Room.all[roomId]);
    }
...
// TODO: rename #addUser to #join and reimplement using DB
...
// TODO: implement...
Room.userLeave = function(user, f){
    for(roomId in Room.all){
        if(Room.all.hasOwnProperty(roomId)){
            Room.all[roomId].removeUser(user);
        }
    }
...
// TODO: implement #countUsers
Room.prototype.countUsers = function(f){
    if(f){ return f(this.users.length); }
};
